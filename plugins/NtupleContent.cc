...
void
NtupleContent::CreateExtraTrgBranches(const std::vector<std::string> & HLTs){
 for( unsigned int ihlt=0; ihlt<HLTs.size(); ihlt++)
...
NtupleContent::ClearBranches(){
  ...
  //gens
  genmu1_pt=0;            genmu1_eta=-99;             genmu1_phi=-99;
  genmu2_pt=0;            genmu2_eta=-99;             genmu2_phi=-99;

  trg_pt.clear();         trg_eta.clear();            trg_phi.clear();
  prb_pt.clear();         prb_eta.clear();            prb_phi.clear();

  tag_pt=0;               tag_eta=-99;                tag_phi=-99;
  tag_isLoose=false;      tag_isMedium=false;         tag_isTight=false;
  tag_isSoft=false;       tag_isHighPt=false;         tag_relIso04=-99;
