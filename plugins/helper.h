...
inline float TrackerEnergy04( const float eta_muon, const float phi_muon, const std::vector<TRK> tracks){
   float energy=0;
   for ( const auto & trk: tracks){
     if ( deltaR(eta_muon,phi_muon,trk.eta(),trk.phi())>0.4)
        continue;    
     energy+=trk.pt();
   }
   return energy;
...
template<typename TRK>
std::pair<bool,unsigned> MatchReco(const std::vector<TRK> & tracks,const float & eta, const float & phi, const double & dr_max){
  double minDR=100;
  unsigned idx;
  for(const auto & trk: tracks){
  ...
  if ( minDR<dr_max) return std::make_pair(true,idx);
